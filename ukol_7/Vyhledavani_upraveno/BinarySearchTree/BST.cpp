...
void BST::Del(Node *pNode)
{
	...
	if (pNode->pLeft == NULL && pNode->pRight == NULL) 
	{
		...
		// uprav odkaz u rodièe na NULL a zruš uzel
	...
	if (pNode->pLeft == NULL || pNode->pRight == NULL) 
	{
		...
		if (pParent == NULL) 
			pRoot = x;               // rodiè je koøen stromu
		// potomka rušeného uzlu navaž na rodièe rušeného uzlu
		else 			
			if (pParent->pLeft == pNode)  // (pParent->key > pNode->key)
				pParent->pLeft = x;
			else 
	...
	// odstranuje se uzel, ktery ma dva potomky => náhrada nejbližším nižším
