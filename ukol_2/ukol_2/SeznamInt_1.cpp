...
int* SeznamInt_1::Copy() const
{
	...
	for (int i = 0; i < this->n; i++)
		tmp[i] = this->p_hodnoty[i];

	return tmp;
}

void SeznamInt_1::realokace()	// metoda roysiri stavajici pole na dvojnasobek soucane velikosti
{
	// vytvorim dvojnasobne dlouhe pole
	...
	//nakopiruji do nej stare hodnoty
	for (int i = 0; i < this->n; i++)
		newArray[i] = this->p_hodnoty[i];

	//pomoci pomocneho ukazatele uvolnim z pameti stare pole a priradim mu nove;
	int* tmp;
	tmp = this->p_hodnoty;
	this->p_hodnoty = newArray;
	delete[] tmp;
